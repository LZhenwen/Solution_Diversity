...
def transfer_num_boost(data):  # transfer num into "NUM"
    ...
    for d in data:
        ...
        if str(id2) in label_dict:
            ...
            pairs.append((input_seq, from_infix_to_prefix(label_dict[str(id2)]), nums, num_pos, answer))
        else:
    ...
    for pair in pairs_trained:
        ...
        if pair[1]:
            ...
            train_pairs.append([input_cell, inputs['input_ids'].squeeze().size(0), output_cell, len(output_cell),
                                pair[2], num_pos, num_stack, pair[4], pair[5], inputs])
        else:
            num_stack = []
            input_cell = indexes_from_sentence(input_lang, pair[0])
            output_cell = indexes_from_sentence(output_lang, pair[1], tree)
...
def prepare_train_batch_together(pairs_to_batch, batch_size, epoch, only_labeled = True):
    ...
    for batch in batches:
        ...
        group_batch = []
        num_value_batch = []
        for i, li, j, lj, num, num_pos, num_stack, _, _, bert_input in batch:
