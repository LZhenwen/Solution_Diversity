...
class MLP(nn.Module):
    ...
    def forward(self, x, y):
        ...
        return torch.sigmoid(torch.matmul(x, y).squeeze())
...
class Bert_Encoder(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, n_layers=2, dropout=0.5):
        ...
        #self.down = nn.Linear(768,300)
        #self.bert_rnn = BertModel.from_pretrained("/ibex/scratch/lianz0a/bert2tree-master/MWPbert-retrained/checkpoint-10000")
        #self.bert_rnn = BertModel.from_pretrained("/home/lianz0a/bert2tree-master/models/epoch_3")
    def forward(self, input_seqs, input_lengths, graph, bert_encoding, hidden=None):
        # Note: we run this all at once (over multiple batches of multiple sequences)
        ...
        #bert_output = self.down(bert_output)        

        problem_output = bert_output.mean(0)
        #pade_outputs = pade_outputs[:, :, :self.hidden_size] + pade_outputs[:, :, self.hidden_size:]  # S x B x H
        return bert_output, problem_output
    def evaluate(self, input_seqs, input_lengths, graph, bert_encoding):
...
class Equation_encoder(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, n_layers=2, dropout=0.05):
        super(Equation_encoder, self).__init__()
    ...
    def forward(self, input_seqs, hidden = None):
        ...
        out = outputs[:, :, :self.hidden_size] + outputs[:, :, self.hidden_size:]  # Sum bidirectional outputs
...
class LSTMEncoderSeq(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, n_layers=2, dropout=0.5):
        super(LSTMEncoderSeq, self).__init__()
...
class TransEncoderSeq(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, n_layers=2, dropout=0.5):
        super(TransEncoderSeq, self).__init__()
        ...
        self.n_layers = n_layers
        self.dropout = dropout
        encoder_layer_1 = nn.TransformerEncoderLayer(d_model=512, nhead=8)
        encoder_norm = nn.LayerNorm(512)
        self.pos_encoder = PositionalEncoding(512, 0.2)
        self.trans = nn.TransformerEncoder(encoder_layer_1, 6, encoder_norm)
        self.embedding = nn.Embedding(input_size, embedding_size, padding_idx=0)
        self.em_dropout = nn.Dropout(dropout)
        #self.gru_pade = nn.LSTM(embedding_size, hidden_size, n_layers, dropout=dropout, bidirectional=True)
        self.gcn = Graph_Module(hidden_size, hidden_size, hidden_size)
    ...
    def forward(self, input_seqs, input_lengths, batch_graph, hidden=None):
        # Note: we run this all at once (over multiple batches of multiple sequences)
        embedded = self.embedding(input_seqs)  # S x B x E
        mask = (input_seqs.transpose(0,1) == 0).bool()
        embedded = self.pos_encoder(embedded)
        embedded = self.em_dropout(embedded)
        src_mask = self.generate_square_subsequent_mask(embedded.size(0))
        #packed = torch.nn.utils.rnn.pack_padded_sequence(embedded, input_lengths)
        #pade_hidden = hidden
        #pade_outputs, pade_hidden = self.gru_pade(packed, pade_hidden)
        #pade_outputs, _ = torch.nn.utils.rnn.pad_packed_sequence(pade_outputs)
        pade_outputs = self.trans(embedded, mask = src_mask, src_key_padding_mask = mask)
