...
def compute_result_weak(test_res, tar_value, output_lang, num_list, num_stack):
...
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, predict, generate, merge, seq_encoder, mlp, seq_optimizer, encoder_optimizer, predict_optimizer, generate_optimizer,
               merge_optimizer, output_lang, num_pos, batch_graph, prob_mask_batch, aug_batch, epoch, english=False):
    ...
    loss_dis = 0
    for idx in range(len(aug_batch)):
        pos = aug_batch[idx]
        #seq_feature = seq_encoder(aug_tensor)
        ...
        for ii in range(len(neg)):
            while neg[ii] == pos[ii]:
                for jj in range(len(neg[ii])):
                    if random.random() < 0.1:
                        if neg[ii][jj] <= 4:
                            op_list = [0, 1, 2, 3]
                            if int(neg[ii][jj]) in op_list:
                                op_list.remove(int(neg[ii][jj]))
                            neg[ii][jj] = random.choice(op_list)
                        if neg[ii][jj] >= 7:
                            num_list = list(range(7, max(neg[ii]) + 1))
                            num_list.remove(int(neg[ii][jj]))
                            if num_list != []:
                                neg[ii][jj] = random.choice(num_list)
        aug_tensor = torch.LongTensor(aug_batch[idx]).transpose(0, 1) 
        neg_tensor = torch.LongTensor(neg).transpose(0, 1) 
        if USE_CUDA:
            aug_tensor = aug_tensor.cuda()
            neg_tensor = neg_tensor.cuda()
        mwp_feature = problem_output[idx]
        pos_feature = seq_encoder(aug_tensor)
        neg_feature = seq_encoder(neg_tensor)
        pos_logits = mlp(mwp_feature.unsqueeze(0).repeat(pos_feature.shape[0], 1), pos_feature).squeeze()
        neg_logits = mlp(mwp_feature.unsqueeze(0).repeat(neg_feature.shape[0], 1), neg_feature).squeeze()
        #pos_logits = torch.sigmoid(torch.matmul(pos_feature.unsqueeze(1), mwp_feature.unsqueeze(-1))).squeeze()
        #neg_logits = torch.sigmoid(torch.matmul(neg_feature.unsqueeze(1), mwp_feature.unsqueeze(-1))).squeeze()
        loss_dis += f.binary_cross_entropy(pos_logits, torch.ones(pos_logits.shape).cuda().float()) + f.binary_cross_entropy(neg_logits, torch.zeros(neg_logits.shape).cuda().float())
    loss_dis = loss_dis / len(aug_batch)
    if random.random() < 0.01:
        print('cls_loss:' ,loss_dis.item())
    ...
    prob_mask = torch.FloatTensor(prob_mask_batch).squeeze().unsqueeze(1).cuda()
    loss = masked_cross_entropy_with_prob(all_node_outputs, target, target_length, prob_mask)
    # loss = loss_0 + loss_1
    loss.backward()
    # clip the grad
...
def train_tree_bertnew(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, predict, generate, merge, seq_encoder, mlp, seq_optimizer, encoder_optimizer, predict_optimizer, generate_optimizer,
               merge_optimizer, output_lang, num_pos, batch_graph, prob_mask_batch, aug_batch, bert_input_batch, epoch, english=False):
    # sequence mask for attention
    seq_mask = []
    max_len = max(input_length)
    for i in input_length:
    ...
    seq_optimizer.zero_grad()
    # Run words through encoder
    encoder_outputs, problem_output = encoder(None, input_length, batch_graph, bert_input_batch)
    loss_dis = 0
    for idx in range(len(aug_batch)):
        pos = aug_batch[idx]
        #seq_feature = seq_encoder(aug_tensor)
...
def train_tree_bert(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, predict, generate, merge, encoder_optimizer, predict_optimizer, generate_optimizer,
               merge_optimizer, output_lang, num_pos, batch_graph, prob_mask_batch, bert_input_batch, english=False):
    # sequence mask for attention
    seq_mask = []
    max_len = max(input_length)
    for i in input_length:
...
def train_tree_prob(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, predict, generate, merge, seq_encoder, encoder_optimizer, predict_optimizer, generate_optimizer,
    ...
    eq_feature = seq_encoder(target[:, :int(target_length[0])])
    logits = torch.sigmoid(torch.matmul(problem_output.squeeze().unsqueeze(0), eq_feature.squeeze().unsqueeze(1)))
    # Prepare input and output variables
    node_stacks = [[TreeNode(_)] for _ in problem_output.split(1, dim=0)]
    ...
    probs_list = f.softmax(all_node_outputs, dim = -1).detach().cpu() #1 * len * N

    p = 1
    for idx, token_number in enumerate(target.detach().cpu()[0]):
        p *= probs_list[0][idx][token_number]

    return p, logits


def train_tree_prob_newbert(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, predict, generate, merge, seq_encoder, encoder_optimizer, predict_optimizer, generate_optimizer,
    ...
    encoder_outputs, problem_output = encoder.evaluate(None, input_length, batch_graph, bert_input)
...
def train_tree_prob_bert(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, predict, generate, merge, encoder_optimizer, predict_optimizer, generate_optimizer,
               merge_optimizer, output_lang, num_pos, batch_graph, bert_input, english=False):
    # sequence mask for attention
    seq_mask = []
    ...
    encoder_outputs, problem_output = encoder(None, input_length, batch_graph, bert_input)
    # Prepare input and output variables
    node_stacks = [[TreeNode(_)] for _ in problem_output.split(1, dim=0)]
...
def evaluate_tree(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, num_pos, batch_graph, beam_size=5, english=False, max_length=MAX_OUTPUT_LENGTH):
    ...
    return [beams[i].out for i in range(len(beams))], problem_output.data

def evaluate_tree_newbert(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, num_pos, batch_graph, bert_input, beam_size=5, english=False, max_length=MAX_OUTPUT_LENGTH):
...
def evaluate_tree_bert(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, num_pos, batch_graph, bert_input, beam_size=5, english=False, max_length=MAX_OUTPUT_LENGTH):
    ...
    return [beams[i].out for i in range(len(beams))]
