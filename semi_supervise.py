...
data_27k = raw_data("data/Math57k.json", id_list)

#group_data = read_json("data/Math_23K_processed.json")


pairs_23k, _, _ = transfer_num(data_23k)
pairs_27k = transfer_num_boost_57k(data_27k)
print('number of 23k:', len(pairs_23k))
print('number of 57k:', len(pairs_27k))
...
temp_pairs = []
for p in pairs_27k:
    temp_pairs.append((p[0], [], p[2], p[3], p[4],'57k',p[5], []))
pairs_27k = temp_pairs
...
fold_size = int(len(pairs_27k) * 0.2)
fold_pairs = []
for split_fold in range(4):
    fold_start = fold_size * split_fold
    fold_end = fold_size * (split_fold + 1)
    fold_pairs.append(pairs_27k[fold_start:fold_end])
fold_pairs.append(pairs_27k[(fold_size * 4):])
...
for fold_t in range(5):
    if fold_t == fold:
        pairs_tested_27k += fold_pairs[fold_t]
    else:
        pairs_trained_27k += fold_pairs[fold_t]



input_lang, output_lang, train_pairs, test_pairs = prepare_data_newbert(pairs_trained_23k + pairs_trained_27k, pairs_tested_23k + pairs_tested_27k, 5, generate_nums,
                                                                copy_nums, tree=True)
# Initialize models
...
for epoch in range(n_epochs):
    ...
    if epoch % 5 == 0 or epoch > n_epochs - 10:
        ...
        print("test_answer_acc_57k", float(value_ac_57k) / eval_total_57k, eval_total_57k)
